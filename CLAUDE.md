...
## Project Overview
Metadata-driven trade surveillance platform demo (Risk Case Manager). Python FastAPI + DuckDB backend, React 19 + TypeScript + Vite frontend. 23 views, 1343 tests (1105 backend + 238 E2E), 8 entities, 5 detection models, 35 guided scenarios, 112 architecture sections.

## Quick Start
```bash
./start.sh                                    # Start app on port 8000
uv run python -m qa test backend              # Run backend tests (1105)
uv run python -m qa test e2e                  # Run E2E Playwright tests (238)
...
- **Tests**: `tests/` — 1105 backend tests + 238 E2E Playwright tests
...
- Progress tracker: `docs/progress.md` (M0-M280 complete)
