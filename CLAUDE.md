...
## Project Overview
Metadata-driven trade surveillance platform demo (Risk Case Manager). Python FastAPI + DuckDB backend, React 19 + TypeScript + Vite frontend. 20 views, 929 tests (705 backend + 224 E2E), 8 entities, 5 detection models, 32 guided scenarios.

## Quick Start
```bash
./start.sh                          # Start app on port 8000
uv run pytest tests/ --ignore=tests/e2e -v   # Run backend tests (705)
uv run pytest tests/e2e/ -v                   # Run E2E Playwright tests (224)
...
- **Tests**: `tests/` — 705 backend tests + 224 E2E Playwright tests
