...
## Project Overview
Metadata-driven trade surveillance platform demo (Risk Case Manager). Python FastAPI + DuckDB backend, React 19 + TypeScript + Vite frontend. 22 views, 1287 tests (1057 backend + 230 E2E), 8 entities, 5 detection models, 34 guided scenarios, 104 architecture sections.

## Quick Start
```bash
./start.sh                                    # Start app on port 8000
uv run python -m qa test backend              # Run backend tests (1057)
uv run python -m qa test e2e                  # Run E2E Playwright tests (230)
...
cd frontend && npm run build                   # Build frontend (973 modules)
...
- **Frontend**: `frontend/` — React 19 + TypeScript + Vite, 22 views, Zustand stores
...
- **Tests**: `tests/` — 1057 backend tests + 230 E2E Playwright tests
...
- Progress tracker: `docs/progress.md` (M0-M268 complete)
