...
- **Branch**: `feature/governance/masking-rbac` — Phase 22 (Masking & RBAC) complete
- **GitHub**: `analytics-platform-demo` repo
- **Tests**: 1287 total (1057 backend + 230 E2E Playwright) — ALL PASSING
- **QA Automation**: `qa/` toolkit — `uv run python -m qa <command>` for test, quality, report, gate, baseline, watch, hooks
- **Frontend**: builds clean — 973 modules (`cd frontend && npm run build`)
- **Views**: 22 (Dashboard, EntityDesigner, MetadataExplorer, SettingsManager, MappingStudio, PipelineMonitor, SchemaExplorer, SQLConsole, ModelComposer, DataManager, UseCaseStudio, RiskCaseManager, AIAssistant, MetadataEditor, RegulatoryMap, Submissions, MedallionOverview, DataOnboarding, DataQuality, ReferenceData, AnalyticsTiers, DataGovernance)
...
- **Architecture**: 104 sections across 22 views, 82% metadata-driven
- **Tour scenarios**: 34 guided scenarios (S1-S34) in 14 categories
- **Operation scripts**: 128 operations across 22 views
- **Golden records**: 301 total (25 product, 6 venue, 220 account, 50 trader)
- **Milestones**: M0-M268 complete
- **Roadmap**: 33 phases across 7 tiers — medallion architecture, data governance, business glossary, migration readiness
- **Latest**: Phase 22 Masking & RBAC complete (M257-M268). MaskingService (partial/tokenize/generalize/redact), RBACService (4 roles), 7 governance API endpoints, audit-aware masking, DataGovernance view (4 tabs), global role switcher, S34 scenario. Next: Phase 23+
