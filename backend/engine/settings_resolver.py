...
class SettingsResolver:
    def resolve(self, setting: SettingDefinition, context: dict[str, str]) -> ResolutionResult:
        ...
        4. Default fallback
        """
        if setting.match_type == "hierarchy":
            matched = self._resolve_hierarchy(setting.overrides, context)
        else:
            matched = self._resolve_multi_dimensional(setting.overrides, context)
    ...
    def _resolve_hierarchy(
        self, overrides: list[SettingOverride], context: dict[str, str]
        ...
        candidates = []
        for ov in overrides:
            if self._all_keys_match(ov.match, context):
    ...
    def _resolve_multi_dimensional(
        self, overrides: list[SettingOverride], context: dict[str, str]
    ) -> SettingOverride | None:
        """Multi-dimensional: count how many dimensions match. Most matches wins, tie-broken by priority."""
        candidates = []
        for ov in overrides:
            match_count = self._count_matching_dimensions(ov.match, context)
            if match_count > 0:
    ...
    def _all_keys_match(self, match: dict[str, str], context: dict[str, str]) -> bool:
        return all(context.get(k) == v for k, v in match.items())

    def _count_matching_dimensions(self, match: dict[str, str], context: dict[str, str]) -> int:
